...
public class VisualiserGIFPreTest {
	...
	public void testShow3D() throws Exception{
		Board3D board1 = new Board3D(5), board2 = new Board3D(5);
		
		game = new Game(board1, board2, player1, player2);
		VisualiserGIF vg = new VisualiserGIF(game);
		game.start(); // tiene barcos pero estan tapados
		game.playGame(vg);

		assertTrue ("Comprueba este test de forma aislada",compareFiles(DIRFILES+"full3D.gif","files/output.gif"));

	}
	
	/* prueba el visualiser usando solo nextShoot de playerRandom */
	@Test
	public void testNextShoot2D() throws Exception{
		Board2D board2d = new Board2D(5);
		IPlayer ip = new PlayerRandom("Rosa", 47);
		Game g = new Game(board2d, board2d, ip, ip);
		VisualiserGIF vg = new VisualiserGIF(g);
		
		ip.putCrafts(board2d);
		do {
			try {
				Coordinate shooted = ip.nextShoot(board2d);
				System.out.println(shooted);
				vg.show();
			}
			catch(CoordinateAlreadyHitException e) {}
		}while(!board2d.areAllCraftsDestroyed());
		vg.close();
		assertTrue ("Comprueba este test de forma aislada",compareFiles(DIRFILES+"foreveralone.gif","files/output.gif"));
	}
	...
	public void testShowEmpty2D() throws IOException {
		game = new Game(board1, board2, player1, player2);
		VisualiserGIF vg = new VisualiserGIF(game);
		vg.show(); // no guardamos una partida hacemos una instantanea de la situcacion inicial.
		vg.close();
	...
	public void testShowEmpty3D() throws IOException {
		Board3D board1 = new Board3D(10), board2 = new Board3D(10);
		game = new Game(board1, board2, player1, player2);
		VisualiserGIF vg = new VisualiserGIF(game);
		vg.show();
		vg.close();
	...
	@Test
	public void testShow2DWithShipsAndPlayGame() throws IOException, InvalidCoordinateException, 
	    NextToAnotherCraftException, OccupiedCoordinateException, CoordinateAlreadyHitException {
		game = new Game(board1, board2, player1, player2);
		VisualiserGIF vg = new VisualiserGIF(game);
		game.start();
		game.playGame(vg); // dentro ya hace vg.show() (varias veces) y vg.close();
		assertTrue ("Comprueba este test de forma aislada",compareFiles(DIRFILES+"Show2DplayGame.gif","files/output.gif"));
	}
